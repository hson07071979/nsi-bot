...
function danhMuc() {
  ...
  (D.open_positions || []).forEach(p => {
    ...
    // entry_px = gia von DA GOM phi mua, theo thang gia hien tai. laiSauPhi() cong phi
    // mua lan nua -> bo phi ra truoc (truoc day lai/lo bi tru phi mua hai lan).
    const von = p.entry_px / 1000 / (1 + PHI_MUA);
    them({
      sym: p.sym, name: tenCua(p.sym), sector: p.sector || nganhCua(p.sym),
      entry: p.entry, entry_px: p.entry_raw ? p.entry_raw / 1000 : von, last: px, sh: p.shares || null,
  ...
  manualGop().trades.filter(t => !t.sell_px).forEach(t => {
    ...
    them({
      ...
      note: t.note || '', nguon: t.chuaDang ? 'nhap' : 'tay', book_id: 'TAY',
    });
  });
...
function lichSuThat() {
  ...
  (F && F.closed || []).forEach(c => out.push({
    ...
    held: c.held, reason: c.reason, peak: c.peak, nguon: 'ghitien',
  }));
...
function dauMuaBan(sym) {
  ...
  // dau cua CHINH nen (co ca gia dieu chinh pxa) dung TRUOC de bo loc trung giu ban day du
  (((D.candles || {})[sym] || {}).marks || []).forEach(x => m.push(x));
  (D.open_positions || []).forEach(p => {
    // dau B = gia THO da khop (nen ve bang gia tho); entry_px la gia von dieu chinh GOM PHI
    if (p.sym === sym) m.push({ t: 'B', d: p.entry, px: ((p.entry_raw || p.entry_px) / 1000).toFixed(2),
                                pxa: (p.entry_px / 1000 / (1 + PHI_MUA)).toFixed(2) });
  });
  const F = pfData();
...
function nhanNguon(n) {
  ...
  if (n === 'ghitien') return '<span class="tag B" style="margin-left:6px" title="Sổ ghi tiến (portfolio.json) — NAV riêng của sổ này">Sổ ghi tiến</span>';
...
function pageSoLenh(root) {
  ...
  const nhatKy = !(F && F.log && F.log.length) ? '' : `
    <h2>Nhật ký sổ ghi tiến <span class="muted" style="font-size:13px;font-weight:400">(portfolio.json — sổ riêng, vốn và số cổ khác sổ hệ thống)</span></h2>
  ...
  <div class="grid kpis">
    ${kpi('Đang nắm giữ', `${dm.length} mã`, [
        `${dm.filter(x => x.book_id === 'HE_THONG').length} sổ hệ thống`,
        dm.some(x => x.book_id === 'SO_GHI_TIEN') ? `${dm.filter(x => x.book_id === 'SO_GHI_TIEN').length} sổ ghi tiến` : '',
        nTay ? `${nTay} anh nhập` : ''].filter(Boolean).join(' · '))}
