...
CFG = dict(
  ...
  # --- LOP MARGIN (chi co o ban demo nay, KHONG co trong engine2.py that) ---
  # margin_map: den thi truong -> (so ma toi da, tran tong von theo % NAV).
  #   1.0 = khong vay. 2.0 = vay bang dung von tu co (don bay 2:1).
  # margin_rate: lai vay theo NAM, tinh tren so du tien AM, theo ngay LICH.
  # margin_call: ty le ky quy toi thieu NAV/tong tai san. Thung thi ban bot
  #   theo gia dong cua cho toi khi ve lai muc an toan — khong doan thi truong.
  margin_map=None, margin_rate=0.0, margin_call=0.30, margin_safe=0.40,
  sector_cap=0.30,
)
...
def run(cfg=None, log=True):
    ...
    phi_lai=0.0; ngay_vay=0; vay_dinh=0.0; don_bay_dinh=1.0; goi_ky_quy=[]
    for i in range(i0,N):
        day=dt.date.fromisoformat(str(cal[i]))
        # --- LAI MARGIN ---
        # Tinh tren so du tien AM, theo ngay LICH (nghi le van tinh lai, dung
        # nhu cong ty chung khoan lam), tru thang vao tien mat.
        if C.get('margin_rate') and cash < 0 and i > i0:
            _dn = (day - dt.date.fromisoformat(str(cal[i-1]))).days
            _l = -cash * C['margin_rate'] / 365.0 * max(_dn, 0)
            cash -= _l; phi_lai += _l
        if cash < 0: ngay_vay += 1; vay_dinh = max(vay_dinh, -cash)
        mv=0.0
        for p in pos.values():
            px=AC[i,p.j]
            mv += (px if not np.isnan(px) else p.epx)*p.sh
        nav=cash+mv
        if mv > 0: don_bay_dinh = max(don_bay_dinh, mv / nav) if nav > 0 else 999
        # --- GOI KY QUY ---
        # Ty le ky quy = von tu co / tong tai san. Thung nguong thi ban bot theo
        # gia dong cua PHIEN NAY, ban vi the LO NANG NHAT truoc, cho toi khi ve
        # muc an toan. Khong phai du doan — la bi ep ban.
        if C.get('margin_map') and mv > 0 and nav > 0 and nav/mv < C['margin_call']:
            _ds=[]
            for _sy,_q in pos.items():
                _px=AC[i,_q.j]
                if np.isnan(_px): continue
                _ds.append((_px/_q.epx-1,_sy,_q,_px))
            _ds.sort()
            for _g,_sy,_q,_px in _ds:
                if nav/mv >= C['margin_safe'] or mv<=0: break
                _got=_q.sh*_px*(1-C['fee_sell']-SS)
                trades.append(dict(sym=_sy,sector=_q.sector,entry=str(cal[_q.ei]),exit=str(cal[i]),
                    entry_px=round(_q.epx/1000,2),exit_px=round(float(_px)/1000,2),held=i-_q.ei,
                    pnl_pct=round((_px/_q.epx-1)*100,2),pnl_vnd=round(_got-_q.sh*_q.epx),
                    reason='Bị gọi ký quỹ',light=R['light'][_q.ei],peak=round(float(_q.peak)*100,1)))
                cash+=_got; mv-=_q.sh*_px; nav=cash+mv; pos.pop(_sy)
                goi_ky_quy.append(str(cal[i]))
        ...
        _MM=C.get('margin_map')
        if _MM: _mn,_mt=_MM.get(R['light'][i],(0,0.0))
        else:   _mn,_mt=C['max_pos_n'],C['max_total']
        # suc mua = tien mat + han muc vay. Phai tinh LAI moi lan dung, khong
        # thi ma thu hai trong cung phien van thay so du cu -> vay qua han muc.
        _bp=lambda: cash+nav*max(0.0,_mt-1.0)
        if smul>0 and _mn>0 and len(pos)<_mn:
            ...
            for sc,j,sym,rmul,bm,pts in rows:
                ...
                if tot/nav>=_mt or len(pos)>=_mn: break
                # Tran nganh scale theo tran tong von: giu NGUYEN muc tap trung
                # tuong doi. De nguyen 30% NAV thi o don bay 2x moi nganh chi
                # nhet duoc dung 1 ma, tu no chan het demo.
                tgt=min(nav*C['base_size']*smul*rmul*bm, nav*C['max_pos'],
                        nav*_mt-tot, nav*C.get('sector_cap',0.30)*max(_mt,1.0)-sec, _bp())
        ...
        for sym,p in list(pos.items()):
            ...
            if add<100 or (px*p.sh+add*px)>nav*C['max_pos'] or add*c2>_bp(): continue
    ...
    return dict(eq=eq,trades=trades,signals=sigs,R=R,blocked=blocked,pos=pos,
                ...
                margin=dict(phi_lai=phi_lai,ngay_vay=ngay_vay,vay_dinh=vay_dinh,
                            don_bay_dinh=don_bay_dinh,goi_ky_quy=goi_ky_quy))
